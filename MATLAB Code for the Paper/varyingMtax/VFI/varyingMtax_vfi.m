%-------------------------------------------------------------------------%
% Solve the growth model with varying marginal tax rate using value
% function iteration optimizing with the Euler equation
%-------------------------------------------------------------------------%
% This program calls the following .m and .mat file(s)
%   * varyingMtax_par.mat: loads in common parameters across solution
%        methods
%   * vMtaxEuler_vfi.m: this m-file solves for the policy function psi(k,z)
...
% xmat       = ksize x zsize matrix, equilibrium taxable income for state
%              grid
% onemat     = ksize x zsize matrix of ones
% taumat     = ksize x zsize matrix, equilibrium marginal tax rate for
%              state grid
% dmat       = ksize x zsize matrix, equilibrium lump sum transfer from the
%              government for state grid
% dtaudkmat  = ksize x zsize matrix, equilibrium derivative of the marginal
%              tax rate with respect to savings for state grid
...
%              consumption next period: [f(k',z')]*u'(c'(k',z',psiprime))
...
xmat = wmat + (rmat - delta*ones(size(rmat))).*kmat ;
onemat = ones(size(xmat)) ;
taumat = tau1*onemat + ...
       ((1/pi)*atan(atau*xmat + btau*onemat) + 0.5*onemat).*(tau2 - tau1) ;
dmat = taumat.*xmat ;
dtaudkmat = ((tau2 - tau1)/pi)*((atau*(rmat - delta*onemat))./...
            (onemat + (atau*xmat + btau*onemat).^2)) ;
% psi_init = zeros(ksize,zsize) ;
% psimat = psi_init ;
% use solution from bechmark case as iniital guess.
psi_init = psimat;
...
while psidist > psitoler && psiiter < psimaxiter
    psiiter  = psiiter + 1 ;
    psiprime = psimat ;
    cprime = wmat + ((1 - delta)*ones(size(rmat)) + rmat).*kmat - ...
             taumat.*xmat - psiprime + dmat ;
    muprime = (cprime.^(-gamma)).*...
              (onemat + (onemat - taumat).*(rmat - delta*onemat) - ...
              dtaudkmat.*xmat) ;
    ...
    for kind = 1:ksize
        for zind = 1:zsize
            ...
            % params  = 1 x 10 vector of parameters to pass in to fsolve
            ...
            params  = [beta delta gamma kt wt rt tau1 tau2 atau btau] ;
            kprinit = (wt + (1+rt-delta)*kt)/2 ;
            % kprmin  = 10^(-10) ;
            % kprmax  = wt + (1 + rt - delta)*kt - 10^(-10) ;
            [kpr,fval_psi] = fsolve(@vMtaxEuler_vfi,kprinit,options,...
