%-------------------------------------------------------------------------%
% Compute current state linearization solution and corresponding two IRFs
% and longer simulation for for growth model with varying marginal tax 
% rates
%-------------------------------------------------------------------------%
% This program calls the following .mat file(s)
%     * varyingMtaxParams.mat: loads common parameter values into memory
%     * varyingMtaxParams.mat: loads common parameter values into memory
%     * varyingMtaxr_vfi.mat: loads in capital paths from VFI IRF1, IRF2, 
%       and simulation
...
% uparams = 1 x 11 vector of parameters to pass into function calculating U
%-------------------------------------------------------------------------%
nx      = 1 ;
ny      = 0 ;
nz      = 1 ;
incr    = 0.00001 ;
uparams = [alpha, beta, delta, gamma, mu, rho, sigma, nx, ny, nz, incr,...
           tau1, tau2, atau, btau] ;
...
% vfierr_rms_csl1 = scalar, root mean squared deviation from VFI solution
%                   for IRF1
% vfierr_rms_csl2 = scalar, root mean squared deviation from VFI solution
%                   for IRF2
% vfierr_rms_csl3 = scalar, root mean squared deviation from VFI solution
%                   for simulation
%-------------------------------------------------------------------------%
...
vfierr_rms_csl1 = sqrt(sum((kirfvec_csl1 - kirfvec_vfi1).^2)/Tirf) ;
vfierr_rms_csl2 = sqrt(sum((kirfvec_csl2 - kirfvec_vfi2).^2)/Tirf) ;
vfierr_rms_csl3 = sqrt(sum((ksimvec_csl3 - ksimvec_vfi3).^2)/Tsim) ;

display([eulerr_rms_csl1,eulerr_mab_csl1 vfierr_rms_csl1;...
    eulerr_rms_csl2,eulerr_mab_csl2 vfierr_rms_csl2;...
    eulerr_rms_csl3,eulerr_mab_csl3 vfierr_rms_csl3]) ;
