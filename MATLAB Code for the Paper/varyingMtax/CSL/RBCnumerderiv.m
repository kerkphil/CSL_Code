function [AA, BB, CC, DD, FF, GG, HH, JJ, KK, LL, MM, TT] = ...
    RBCnumerderiv(funcname,theta0,params)
...
ny   = params(9) ;
nz   = params(10) ;
incr = params(11) ;    %increment to use in calculating numerical derivatives
...
for i = 1:leng
    if i<3*nx+2*ny+1
        big(:,i) = theta0(i)*(funcname(dev(i,:)',params)-T0)./(1+T0);
    else
        big(:,i) = (funcname(dev(i,:)',params)-T0)./(1+T0);
    end
end
big = big/incr;
