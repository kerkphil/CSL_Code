...
%     * varyingMtax_d1.mod: runs the dynare .mod file for the 1st order
%          approximation
...
%-------------------------------------------------------------------------%
clear ;
%cd('/Users/rwe2/Documents/BYU Economics/Paper Ideas/CSL/MatLab/varyingMtax/Dynare1') ;
...
%-------------------------------------------------------------------------%
dynare varyingMtax_d1.mod noclearall ;
a0 = oo_.dr.ys(2,1) ;
a1 = oo_.dr.ghx(2,1) ;
a2 = oo_.dr.ghx(2,2) ;
a3 = oo_.dr.ghu(2,1) ;
...
for tind = 1:Tirf
    ...
    ktp2v_1 = (b0 + b1*ktp1_1)*ones_z + b2*zvec' ;
    ...
    ktp2v_2 = (b0 + b1*ktp1_2)*ones_z + b2*zvec' ;
...
for tind = 1:Tsim
    ...
    ktp2v_3 = (b0 + b1*ktp1_3)*ones_z + b2*zvec' ;
...
% vfierr_rms_dy11 = scalar, root mean squared deviation from VFI solution
%                   for IRF1
% vfierr_rms_dy12 = scalar, root mean squared deviation from VFI solution
%                   for IRF2
% vfierr_rms_dy13 = scalar, root mean squared deviation from VFI solution
%                   for simulation
%-------------------------------------------------------------------------%
...
vfierr_rms_dy11 = sqrt(sum((kirfvec_dy11 - kirfvec_vfi1).^2)/Tirf) ;
vfierr_rms_dy12 = sqrt(sum((kirfvec_dy12 - kirfvec_vfi2).^2)/Tirf) ;
vfierr_rms_dy13 = sqrt(sum((ksimvec_dy13 - ksimvec_vfi3).^2)/Tsim) ;

display([eulerr_rms_dy11,eulerr_mab_dy11,vfierr_rms_dy11;...
    eulerr_rms_dy12,eulerr_mab_dy12,vfierr_rms_dy12;...
    eulerr_rms_dy13,eulerr_mab_dy13,vfierr_rms_dy13]) ;
