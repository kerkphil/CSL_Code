...
% longer simulation for growth model with varying marginal tax rates
%-------------------------------------------------------------------------%
% NOTE: You must change the initial values in varyingMtax_d1.mod any time
%       you change parameter values in varyingMtax_par.m
%
% This program calls the following .mod and .mat file(s)
%     * varyingMtax_d2.mod: runs the dynare .mod file for the 2nd order
%          approximation
%     * varyingMtax_par.mat: loads common parameter values into memory
%     * varyingMtax_vfi.mat: loads in capital paths from VFI IRF1, IRF2,
%          and simulation
%-------------------------------------------------------------------------%
...
%-------------------------------------------------------------------------%
clear ;
%cd('/Users/rwe2/Documents/BYU Economics/Paper Ideas/CSL/MatLab/varyingMtax/Dynare2') ;
...
%-------------------------------------------------------------------------%
dynare varyingMtax_d2.mod noclearall ;
...
%                k_1 = 1.1*kss
% ksimvec_vfi3 = 1 x Tsim vector, simulate time series of the aggregate
%                capital stock using closed form solution
%-------------------------------------------------------------------------%
load '../ParamSet/varyingMtax_par.mat' beta alpha delta gamma L mu rho ...
     sigma tau1 tau2 atau btau Tirf Tsim kss zvec zTrans zirfvec1 ...
     zirfvec2 zsimvec3 ;
load '../VFI/varyingMtax_vfi.mat' kirfvec_vfi1 kirfvec_vfi2 ksimvec_vfi3 ;
zsize = length(zvec) ;
...
% ones_z       = 1 x znodes vector of ones
...
%                for IRF2
% tautp1v_1    = 1 x znodes vector of potential marginal tax rates next
%                period for IRF1
% tautp1v_2    = 1 x znodes vector of potential marginal tax rates next
%                period for IRF2
% dtautp1v_1   = 1 x znodes vector of potential derivative of tau next
%                period for IRF1
% dtautp1v_2   = 1 x znodes vector of potential derivative of tau next
%                period for IRF2
...
for tind = 1:Tirf
    ...
    ktp2v_1 = (b0 + b1*ktp1_1 * b3*ktp1_1^2)*ones_z + b2*zvec' + ...
              b4*ktp1_1*zvec' + b5*(zvec'.^2) ;
    ctp1v_1 = wtp1v_1 + ktp1_1*((1-delta)*ones_z + rtp1v_1) - ktp2v_1 ;
    tautp1v_1 = tau1*ones_z + ...
        ((1/pi)*atan(atau*(wtp1v_1 + (rtp1v_1 - delta*ones_z).*ktp1_1) ...
        + btau*ones_z) + 0.5*ones_z).*(tau2 - tau1) ;
    dtautp1v_1 = ((tau2 - tau1)/pi)*...
        ((atau*(rtp1v_1 - delta*ones_z))./(ones_z + ...
        (atau*(wtp1v_1 + (rtp1v_1 - delta*ones_z)*ktp1_1) + ...
        btau*ones_z).^2)) ;
    Emutp1_1 = sum(zprobs_1.*(ctp1v_1.^(-gamma)).*...
        (ones_z + (ones_z - tautp1v_1).*(rtp1v_1 - delta*ones_z) ...
        - dtautp1v_1.*(wtp1v_1 + (rtp1v_1 - delta*ones_z).*ktp1_1))) ;
    ...
    ktp2v_2 = (b0 + b1*ktp1_2 * b3*ktp1_2^2)*ones_z + b2*zvec' + ...
              b4*ktp1_2*zvec' + b5*(zvec'.^2) ;
    ctp1v_2 = wtp1v_2 + ktp1_2*((1-delta)*ones_z + rtp1v_2) - ktp2v_2 ;
    tautp1v_2 = tau1*ones_z + ...
        ((1/pi)*atan(atau*(wtp1v_2 + (rtp1v_2 - delta*ones_z).*ktp1_2) ...
        + btau*ones_z) + 0.5*ones_z).*(tau2 - tau1) ;
    dtautp1v_2 = ((tau2 - tau1)/pi)*...
        ((atau*(rtp1v_2 - delta*ones_z))./(ones_z + ...
        (atau*(wtp1v_2 + (rtp1v_2 - delta*ones_z)*ktp1_2) + ...
        btau*ones_z).^2)) ;
    Emutp1_2 = sum(zprobs_2.*(ctp1v_2.^(-gamma)).*...
        (ones_z + (ones_z - tautp1v_2).*(rtp1v_2 - delta*ones_z) ...
        - dtautp1v_2.*(wtp1v_2 + (rtp1v_2 - delta*ones_z).*ktp1_2))) ;
...
%                for simulation
% tautp1v_3    = 1 x znodes vector of potential marginal tax rates next
%                period for simulation
% dtautp1v_3   = 1 x znodes vector of potential derivative of tau next
%                period for simulation
...
for tind = 1:Tsim
    ...
    ktp2v_3 = (b0 + b1*ktp1_3 + b3*ktp1_3^2)*ones_z + b2*zvec' + ...
              b4*ktp1_3*zvec' + b5*(zvec'.^2) ;
    ctp1v_3 = wtp1v_3 + ktp1_3*((1-delta)*ones_z + rtp1v_3) - ktp2v_3 ;
    tautp1v_3 = tau1*ones_z + ...
        ((1/pi)*atan(atau*(wtp1v_3 + (rtp1v_3 + delta*ones_z).*ktp1_3) ...
        + btau*ones_z) + 0.5*ones_z).*(tau2 - tau1) ;
    dtautp1v_3 = ((tau2 - tau1)/pi)*...
        ((atau*(rtp1v_3 - delta*ones_z))./(ones_z + ...
        (atau*(wtp1v_3 + (rtp1v_3 - delta*ones_z)*ktp1_3) + ...
        btau*ones_z).^2)) ;
    Emutp1_3 = sum(zprobs_3.*(ctp1v_3.^(-gamma)).*...
        (ones_z + (ones_z - tautp1v_3).*(rtp1v_3 - delta*ones_z) ...
        - dtautp1v_3.*(wtp1v_3 + (rtp1v_3 - delta*ones_z).*ktp1_3))) ;
...
% vfierr_rms_dy21 = scalar, root mean squared deviation from VFI solution
%                   for IRF1
% vfierr_rms_dy22 = scalar, root mean squared deviation from VFI solution
%                   for IRF2
% vfierr_rms_dy23 = scalar, root mean squared deviation from VFI solution
%                   for simulation
%-------------------------------------------------------------------------%
...
vfierr_rms_dy21 = sqrt(sum((kirfvec_dy21 - kirfvec_vfi1).^2)/Tirf) ;
vfierr_rms_dy22 = sqrt(sum((kirfvec_dy22 - kirfvec_vfi2).^2)/Tirf) ;
vfierr_rms_dy23 = sqrt(sum((ksimvec_dy23 - ksimvec_vfi3).^2)/Tsim) ;

display([eulerr_rms_dy21,eulerr_mab_dy21,vfierr_rms_dy21;...
    eulerr_rms_dy22,eulerr_mab_dy22,vfierr_rms_dy22;...
    eulerr_rms_dy23,eulerr_mab_dy23,vfierr_rms_dy23]) ;
