...
%        methods
%   * psisolveStd_vfi.m: this m-file solves for the policy function
%        psi(k,z) inside each iteration of the value function iteration
%-------------------------------------------------------------------------%
...
%-------------------------------------------------------------------------%
clear ;
%cd('/Users/rwe2/Documents/BYU Economics/Paper Ideas/CSL/MatLab/StdInfHor/VFI') ;
...
while Vdist > Vtoler && Viter < Vmaxiter
    ...
    for kind = 1:ksize
        for zind = 1:zsize
            ...
            [kpr,~] = fmincon(@psisolveStd_vfi,kprinit,[],...
