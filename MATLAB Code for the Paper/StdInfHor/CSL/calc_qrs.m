...
%    if solvesylv == 1;  %solve using generalized Sylvester equation method
%        [QQ, ~, info] = gsylvester(FF,eye(nz),-(LL*NN + MM),FF*PP+GG,NN,FF,2);
%    else
       LLNN_plus_MM = LL*NN + MM;
%        if isGpuAvailable == 1
%            temp = [ DD(:) LLNN_plus_MM(:) ];
%            temp = gpuArray(temp);
%            VVtemp = gpuArray(VV);
%            QQSS_vec = - VVtemp \ temp;
%            QQSS_vec = gather(QQSS_vec);
%        else
           QQSS_vec = - VV \ [ DD(:)
                               LLNN_plus_MM(:) ];
%       end
